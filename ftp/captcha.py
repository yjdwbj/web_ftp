...
#fp = mf.FontProperties(fname='/usr/share/fonts/truetype/dejavu/DejaVuSerif-Bold.ttf')
#flen = len(flst)-1
...
def get_code():
    ...
    img = toimage(make_text_image(WIDTH,HEIGHT,0,-3,randint(25,28),\
            127,str(txt)))
    img.save(streamio,'PNG')
    val = streamio.getvalue()
    streamio.close()
    return (txt,base64.b64encode(val))
...
def make_text_image(width,height,x,y,size,th,text):
    ...
    return ndimage.gaussian_filter(np.array(img),sigma=0.6)
